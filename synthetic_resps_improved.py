...
class BankReviewsAnalyzer:
    ...
    def load_reviews(self, file) -> None:
        """
        Загрузка отзывов из Excel файла с настраиваемым форматом

        Args:
            file: Файл Excel с отзывами о банках
        """
        try:
            self.reviews_data = pd.read_excel(file)
            ...
            if 'bank' in self.reviews_data.columns:
                ...
                st.write(f"Найдено {len(self.banks)} тем/категорий: {', '.join(self.banks[:5] if len(self.banks) > 5 else self.banks)}...")

            # Базовый анализ
            self._analyze_reviews()

        except Exception as e:
            st.error(f"Ошибка при загрузке отзывов: {str(e)}")
...
class RespondentsMarketplace:
    ...
    def load_questions(self, file) -> List[Dict]:
        """
        Загрузка вопросов из Excel файла

        Args:
            file: Excel файл с вопросами
    ...
    def load_bank_reviews(self, file) -> bool:
        """
        Загрузка и анализ отзывов о банках

        Args:
            file: Excel файл с отзывами

        Returns:
            True если загрузка прошла успешно, иначе False
        """
        try:
            self.reviews_analyzer.load_reviews(file)
...
"""
    ...
    def generate_answer(self, persona: Dict, question: Dict,
        ...
        cache_key = f"{json.dumps(persona, sort_keys=True, cls=NumpyEncoder)}_{json.dumps(question, sort_keys=True)}_{model}_{api_preference}_{temperature}"
    ...
    def run_generation_batch(self, personas, questions, max_workers=3, api_preference=None, use_enhanced=True):
        ...
        progress_text.text("Генерация ответов: 0%")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_task):
                ...
                progress_value = completed / total_items
                progress_bar.progress(progress_value)
                progress_text.text(f"Генерация ответов: {int(progress_value*100)}%")
    ...
    def export_to_excel(self, personas: List[Dict], questions: List[Dict], answers: List[Dict], output_file: str) -> str:
        """
        ...
        try:
            ...
            # Создаем BytesIO объект для сохранения Excel файла
    ...
    def visualize_demographics(self, personas):
        """
        ...
        st.subheader("Демографический и финансовый анализ респондентов")

        # Создаем две колонки для визуализаций
        ...
        with col2:
            ...
            sns.histplot(persona_df['Возраст'], kde=True, ax=ax2)
            ax2.set_xlabel('Возраст')
            ax2.set_ylabel('Количество')
            st.pyplot(fig2)

        # Распределение по регионам
        st.write("Распределение по регионам")
        region_counts = persona_df['Регион'].value_counts().head(8)
        fig3, ax3 = plt.subplots(figsize=(10, 5))
        sns.barplot(x=region_counts.index, y=region_counts.values, ax=ax3)
        ax3.set_title('Распределение по регионам (топ 8)')
        ...
        st.write("Уровень финансовой грамотности")
        ...
        sns.barplot(x=sorted_literacy.index, y=sorted_literacy.values, ax=ax4)
        ax4.set_title('Уровень финансовой грамотности')
        ax4.set_xlabel('Уровень')
        ax4.set_ylabel('Количество')
        ax4.tick_params(axis='x', rotation=45)
        st.pyplot(fig4)

        # Создаем две колонки для визуализаций отношения к финансам
        col3, col4 = st.columns(2)

        # Доверие к банкам
        with col3:
            st.write("Доверие к банкам")
            ...
            sns.barplot(x=sorted_trust.index, y=sorted_trust.values, ax=ax5)
            ax5.set_title('Доверие к банкам')
            ax5.set_xlabel('Уровень доверия')
            ax5.set_ylabel('Количество')
            st.pyplot(fig5)

        # Отношение к кредитам
        with col4:
            st.write("Отношение к кредитам")
            ...
            sns.barplot(x=sorted_loan.index, y=sorted_loan.values, ax=ax6)
            ax6.set_title('Отношение к кредитам')
            ax6.set_xlabel('Отношение')
            ax6.set_ylabel('Количество')
            st.pyplot(fig6)


# Функция для сохранения настроек в файл
def save_config_to_file(config, file_path="financial_respondents_config.json"):
...
def load_config_from_file(file_path="financial_respondents_config.json"):
    ...
    try:
        ...
        st.error(f"Ошибка при загрузке настроек: {e}")
        return None

# Функция для создания и настройки интерфейса Streamlit
def main():
    # Заголовок приложения
    st.title("Synthetica Financial: Симулятор финансовых респондентов")
    st.caption("Генерация реалистичных ответов респондентов с разным уровнем финансовой грамотности")
    ...
    # Создаем вкладки для разных этапов работы
    tabs = st.tabs(["Настройки", "Генерация респондентов", "Результаты"])
    
    with tabs[0]:
        st.header("Настройка API ключей")
        ...
        api_key_claude = st.text_input(
            ...
            type="password",
            help="Введите API ключ Anthropic Claude. Обязателен для работы."
        )
        ...
        api_key_openai = st.text_input(
            ...
            type="password",
            help="При наличии ключа OpenAI можно использовать их модели или переключаться между провайдерами."
        )
        ...
        questions_file = st.file_uploader(
            "Загрузите Excel с вопросами", 
            type=["xlsx", "xls"],
            help="Файл должен содержать столбец 'question' с текстами вопросов."
        )
        
        if questions_file:
            st.success(f"Загружен файл с вопросами: {questions_file.name}")
        
        # Элемент для загрузки файла с отзывами о банках
        reviews_file = st.file_uploader(
            "Загрузите Excel с отзывами о банках (опционально)", 
            type=["xlsx", "xls"],
            help="Файл должен содержать отзывы о банках с рейтингами."
        )
        
        if reviews_file:
            st.success(f"Загружен файл с отзывами: {reviews_file.name}")
        ...
        with col1:
            ...
            api_preference = st.radio(
                ...
                index=0 if not saved_config else 
                    1 if saved_config.get('api_preference') == 'openai' else 
                    2 if saved_config.get('api_preference') is None else 0
            )
        ...
        with col2:
            ...
            use_enhanced = st.checkbox(
                ...
                help='Добавляет когнитивные и эмоциональные факторы в ответы'
            )
        ...
        output_format = st.radio(
            ...
            format_func=lambda x: x[0],
            index=0 if not saved_config or saved_config.get('output_format') == 'excel' else 1
        )
        ...
        if st.button('Сохранить настройки'):
            config = {
                ...
                'api_preference': api_preference[1] if isinstance(api_preference, tuple) else api_preference,
                'num_threads': num_threads,
                'use_enhanced': use_enhanced,
                'visualize': visualize_data,
                'output_format': output_format[1] if isinstance(output_format, tuple) else output_format,
    ...
    # Вкладка генерации респондентов
    with tabs[1]:
        st.header("Генерация респондентов")
        
        # Проверка необходимых условий
        missing_requirements = []
        
        if not api_key_claude and not api_key_openai:
            missing_requirements.append("API ключ (Claude или OpenAI)")
        
        if not questions_file:
            missing_requirements.append("Файл с вопросами")
        
        if missing_requirements:
            st.warning(f"Для продолжения необходимо указать: {', '.join(missing_requirements)}")
        else:
            ...
            # Если есть файл с отзывами, загружаем его
            if reviews_file:
                marketplace.load_bank_reviews(reviews_file)
            
            # Если есть файл с вопросами, загружаем его
            questions = []
            if questions_file:
                questions = marketplace.load_questions(questions_file)
                st.success(f"Загружено {len(questions)} вопросов")
            
            # Кнопка для генерации случайных респондентов
            if st.button("Сгенерировать случайных респондентов"):
                with st.spinner("Генерация персон..."):
                    personas = [marketplace.generate_persona() for _ in range(num_respondents)]
                    
                    # Сохраняем в session_state
                    st.session_state.personas = personas
                    st.session_state.questions = questions
                    
                    st.success(f"Сгенерировано {len(personas)} респондентов")
                    
                    # Отображаем демографию
                    if visualize_data:
                        marketplace.visualize_demographics(personas)
                    
                    # Показываем демо-информацию о респондентах
                    st.subheader("Информация о респондентах")
                    for i, persona in enumerate(personas):
                        with st.expander(f"Респондент #{i+1}: {persona['Пол']}, {persona['Возраст']} лет, {persona['Город']}"):
                            st.write(f"**Финансовая грамотность:** {persona['Финансовый профиль']['Уровень финансовой грамотности']}")
                            
                            products = [p for p, used in persona['Финансовый профиль'].get('Используемые продукты', {}).items() if used]
                            if products:
                                st.write(f"**Используемые продукты:** {', '.join(products)}")
                            
                            if 'Финансовые цели' in persona['Финансовый профиль']:
                                st.write(f"**Финансовые цели:** {', '.join(persona['Финансовый профиль']['Финансовые цели'])}")
            
            # Кнопка для запуска генерации ответов (если есть персоны)
            if 'personas' in st.session_state and st.session_state.personas:
                if st.button("Запустить генерацию ответов"):
                    with st.spinner("Генерация ответов... Это может занять некоторое время."):
                        # Получаем настройки API из radio button
                        api_pref = api_preference[1] if isinstance(api_preference, tuple) else api_preference
                        
                        # Запускаем генерацию
                        answers = marketplace.run_generation_batch(
                        ...
                        output_extension = '.xlsx' if output_format[1] == 'excel' else '.json'
                        if not output_file.endswith(output_extension):
                            output_file = output_file.replace('.xlsx', '').replace('.json', '') + output_extension
                        
                        # Экспортируем результаты
                        if output_format[1] == 'excel':
                            excel_buffer = marketplace.export_to_excel(
                                st.session_state.personas,
                                st.session_state.questions,
                                answers,
                                output_file
                            ...
                            # Сохраняем буфер в session_state
                        ...
                        # Переходим к вкладке результатов
                        st.success("Генерация завершена! Перейдите на вкладку 'Результаты'")
    ...
    with tabs[2]:
        if 'answers' in st.session_state and st.session_state.answers:
            ...
            # Анализ результатов
            if 'marketplace' in st.session_state:
                report = st.session_state.marketplace.analyze_results(
                    st.session_state.personas,
                    st.session_state.questions,
                ...
                # Визуализации, если нужно
                if 'visualize' in st.session_state and st.session_state.visualize:
                    st.session_state.marketplace.visualize_demographics(st.session_state.personas)
                ...
                if 'excel_buffer' in st.session_state:
                    ...
                    st.download_button(
                        ...
                        data=excel_data,
                        file_name=output_file if output_file.endswith('.xlsx') else output_file + '.xlsx',
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                    )
                
                if 'json_str' in st.session_state:
                    # Для JSON файла
                    st.download_button(
                        label="📥 Скачать JSON файл",
                        data=st.session_state.json_str,
                        file_name=output_file if output_file.endswith('.json') else output_file + '.json',
                        mime="application/json"
                    )
                
                # Примеры ответов
                st.subheader("Примеры сгенерированных ответов")
                
                # Выбираем несколько случайных ответов для отображения
                sample_answers = random.sample(
                    st.session_state.answers, 
                    min(5, len(st.session_state.answers))
                )
                
                for i, answer in enumerate(sample_answers):
                    persona_id = answer["persona_id"]
                    persona = next(
                        (p for p in st.session_state.personas if p.get("ID", persona_id) == persona_id),
                        st.session_state.personas[persona_id-1]
                    )
                    
                    literacy_level = persona.get("Финансовый профиль", {}).get("Уровень финансовой грамотности", "средний")
                    
                    with st.expander(f"Ответ #{i+1}: {persona['Пол']}, {persona['Возраст']} лет, уровень: {literacy_level}"):
                        st.markdown(f"**Вопрос:** {answer['question']['text']}")
                        st.markdown(f"**Ответ:**")
                        st.markdown(answer["text"])
        else:
            st.info("Сначала сгенерируйте ответы на вкладке 'Генерация респондентов'")
    
    # Информация о приложении
    with st.expander("О приложении"):
        st.markdown("""
        ### Synthetica Financial: Симулятор финансовых респондентов
        
        Это приложение позволяет генерировать реалистичные ответы виртуальных респондентов с разным уровнем финансовой грамотности на вопросы о банковских услугах и финансах.
        
        **Особенности:**
        
        - Генерация разнообразных демографических профилей
        - Разные уровни финансовой грамотности (от отсутствия знаний до эксперта)
        - Учет когнитивных искажений и эмоциональных факторов
        - Региональные, лингвистические и психологические особенности речи
        - Реалистичная непоследовательность в ответах
        
        **Как использовать:**
        1. Укажите API-ключ Claude или OpenAI
        2. Загрузите Excel файл с вопросами (колонка 'question' обязательна)
        3. Опционально загрузите файл с отзывами о банках для обогащения ответов
        4. Настройте количество и параметры респондентов
        5. Запустите генерацию ответов
        6. Скачайте результаты в Excel или JSON
        
        **Требования к файлу с вопросами:**
        Файл должен содержать колонку 'question' с текстами вопросов. Опционально могут быть колонки:
        - 'type': тип вопроса ('open', 'single', 'multiple')
        - 'topic': тема вопроса ('кредиты', 'карты', 'вклады', 'инвестиции', 'общие', 'онлайн-сервисы')
        - 'options': варианты ответов через запятую (для типов 'single' и 'multiple')
        - 'context': дополнительный контекст для вопроса
        
        **Требования к файлу с отзывами:**
        Файл должен содержать отзывы с рейтингами. Колонки:
